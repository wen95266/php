<?php
require 'config.php';
if (!isset($_SESSION['uid']) || !$_SESSION['is_admin']) die(json_encode(['error'=>'仅管理员可减分']));
...
$newscore = $u['score'] - abs($value);
$db->query("UPDATE users SET score=$newscore WHERE id={$u['id']}");
$db->query("INSERT INTO score_history (user_id, change_val, after_val, change_type, remark, create_time) VALUES ({$u['id']}, -$value, $newscore, 'admin_sub', '管理员减分', NOW())");
