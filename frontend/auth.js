...
document.addEventListener('DOMContentLoaded', () => {
    // --- MODIFICATION: Use absolute path for API URL ---
    const API_AUTH_URL = '/backend/api/auth.php'; // Assuming 'backend' is in the web root
    ...
    async function handleAuthResponse(data, messagesElement, redirectUrl = null) {
        if (data && data.success) {
            ...
            if (typeof window.updateUserInfoDisplay === 'function') {
                ...
                console.warn("window.updateUserInfoDisplay function is not defined globally.");
            }
    ...
    if (loginForm) {
        loginForm.addEventListener('submit', async (e) => {
            ...
            try {
                ...
                const data = await response.json(); // This line might throw if response is not JSON
                handleAuthResponse(data, loginMessagesDiv, 'lobby.html');
            } catch (error) {
                console.error("Login fetch/json error:", error);
                showAuthMessage(loginMessagesDiv, "登录请求失败或服务器响应格式错误。", "error");
            } finally {
    ...
    if (registerForm) {
        registerForm.addEventListener('submit', async (e) => {
            ...
            try {
                ...
                showAuthMessage(registerMessagesDiv, "注册请求失败或服务器响应格式错误。", "error");
            } finally {
                if(submitButton) submitButton.disabled = false;
            }
        });
    ...
    window.checkUserAuthentication = async function() {
        ...
        try {
            ...
            // --- MODIFICATION: Check response content type before parsing JSON ---
            ...
            if (contentType && contentType.indexOf("application/json") !== -1) {
                const data = await response.json();
                console.log("checkAuth response data (JSON):", data);
                ...
                console.error("checkAuth received non-JSON response (likely HTML error page):", htmlError.substring(0, 500) + "..."); // Log first 500 chars
                localStorage.removeItem('userData');
                if (typeof window.updateUserInfoDisplay === 'function') window.updateUserInfoDisplay();
                return { success: false, isAuthenticated: false, message: '认证服务响应格式错误，请检查服务器日志。' };
            }
        } catch (error) { // Catch network errors or if response.json() itself fails after a non-JSON contentType
            console.error("checkUserAuthentication fetch/processing error:", error); // This is where your original error likely lands
            localStorage.removeItem('userData');
            if (typeof window.updateUserInfoDisplay === 'function') window.updateUserInfoDisplay();
